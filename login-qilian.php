...
$_SESSION['WPOA']['PROVIDER'] = 'Qilian';
define('HTTP_UTIL', get_option('wpoa_http_util'));
define('CLIENT_ENABLED', get_option('wpoa_qilian_api_enabled'));
define('CLIENT_ID', get_option('wpoa_qilian_api_id'));
define('CLIENT_SECRET', get_option('wpoa_qilian_api_secret'));
define('REDIRECT_URI', rtrim(site_url(), '/') . '/');
define('SCOPE', ''); // PROVIDER SPECIFIC: 'r_basicprofile' is the minimum scope required to get the user's id from Qilian
define('URL_AUTH', "https://forum.qilian.jp/oauth/authorize?");
define('URL_TOKEN', "https://forum.qilian.jp/oauth/token?");
define('URL_USER', "https://forum.qilian.jp/api/v3/users/me?");
...
function get_oauth_token($wpoa) {
	...
	// parse the result:
  $result_obj = json_decode($result, true); // PROVIDER SPECIFIC: Qilian encodes the access token result as json by default
	$access_token = $result_obj['access_token']; // PROVIDER SPECIFIC: this is how Qilian returns the access token KEEP THIS PROTECTED!
	$expires_in = $result_obj['expires_in']; // PROVIDER SPECIFIC: this is how Qilian returns the access token's expiration
...
function get_oauth_identity($wpoa) {
  ...
  // set the access token param:
	$params = array(
		'access_token' => $_SESSION['WPOA']['ACCESS_TOKEN'], // PROVIDER SPECIFIC: the access token is passed to Qilian using this key name
	);
  $url_params = http_build_query($params);
	...
	switch (strtolower(HTTP_UTIL)) {
		case 'curl':
			...
			curl_setopt($curl, CURLOPT_HTTPHEADER, array('x-li-format: json')); // PROVIDER SPECIFIC: we must specify json or else Qilian will encode the result as xml by default // PROVIDER NORMALIZATION: PayPal/Reddit require that we send the access token via a bearer header, PayPal also requires a Content-Type: application/json header...
			curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
			$result = curl_exec($curl);
		...
		case 'stream-context':
			...
			$opts = array('http' =>
				array(
					...
					'header'  => "Authorization: Bearer " . $_SESSION['WPOA']['ACCESS_TOKEN'] . "\r\n" . "x-li-format: json\r\n", // PROVIDER SPECIFIC: we must specify json or else Qilian will encode the result as xml by default
				)
      );
			$context = $context  = stream_context_create($opts);
			$result = @file_get_contents($url, false, $context);
  ...
  $oauth_identity['provider'] = $_SESSION['WPOA']['PROVIDER'];
	$oauth_identity['id'] = $result_obj['user']['id']; // PROVIDER SPECIFIC: this is how Qilian returns the user's unique id
	$oauth_identity['email'] = $result_obj['user']['email']; //PROVIDER SPECIFIC: this is how Qilian returns the email address
